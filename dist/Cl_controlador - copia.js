...
export default class Cl_controlador {
    ...
    // En Cl_controlador.ts
    editJurado({ dtJurado, callback, }) {
        this.modelo.editJurado({
            dtJurado,
            // Interceptamos el callback del modelo
            callback: (error) => {
                if (!error) {
                    // ⭐ Orden de refresco a la Vista Principal (que ahora propaga la orden)
                    this.vista.refresh();
                    console.log("✅ CONTROLADOR - Refresco de vista ejecutado.");
                }
                // Llamamos al callback original para mostrar la alerta
                callback(error);
            },
        });
    }
